...
response=$(curl -s -b cookies.txt -X POST "$AWSURL/upload/get-url" \
...
# Upload the file
curl -s -X PUT "$UPLOAD_URL" --upload-file "$FILENAME" -H "Content-Type: video/mp4"

echo "Upload complete, starting encode..."

# Start transcoding
# curl -s -b cookies.txt -X POST "$AWSURL/start-encode" \
#   -H "Content-Type: application/json" \
