...
async function getVideoById(videoId) {
  ...
  return await cachedFetch(
    ...
    60 * 60 // Cache for 1 hour, or until invalidated
  );
}
...
async function fetchVideos({
  ...
  const cacheKey = `videos:${userId}:${upld_before}:${upld_after}:${limit}:${JSON.stringify(lastKey)}`;
  return await cachedFetch(
    cacheKey,
    async () => {
...
async function deleteUserVideo(videoId) {
  try {
    ...
    invalidateCache(`video:${videoId}`);
    // You should also invalidate any list queries that might contain this video.
    // This is more complex and depends on your application logic.
...
async function deleteVideo(video) {
  try {
    ...
    // Invalidate the cache for the video list
    invalidateCache(`videos:${video.userId}`);
